...
import numpy as np
from envs.env_dense_decay_reward import VideoEnv
...
def eval(env, model, eval_episode_num, visualize_index, cfg, epoch=0):
    ...
    for seed in range(eval_episode_num):
        ...
        if seed in visualize_index:
            env.set_options([{"eval_id": seed, "vis_flag": True, "epoch": epoch}])
        else:
...
if __name__ == "__main__":
    ...
    model = ALGOS[cfg["train"]["algorithm"]](
            ...
            batch_size=cfg["train"]["batch_size"], # eliminate for A2C
            n_epochs=10, # eliminate for A2C
            gamma=0.99,
            gae_lambda=0.95,
