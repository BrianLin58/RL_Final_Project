...
def run_evaluation(model_path, config_path):
    ...
    for seed in range(eval_episode_num):
        try:
            ...
            video_name = f"GOT-10k_Val_{(seed + 1):06}"
            episode_results[video_name] = ep_reward
        except:
            print(f"[WARNING] Error when handling {seed}")

    print(f"\nFinal Average Reward: {total_reward / eval_episode_num:.4f}")

    output_file = f"{config_path.split('/')[-1].split('.')[0]}_avgReward_{total_reward / eval_episode_num}.json"
