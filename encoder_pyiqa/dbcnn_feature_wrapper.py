...
class DBCNNWithFeature(DBCNN):
    ...
    def _extract_backbone_features(self, X):
        """Shared helper to get X1, X2 feature maps with matched spatial size."""
        X = self.preprocess(X)  # ImageNet mean/std
    ...
    def forward_gap_feature(self, X, use_x1=True, use_x2=True):
        """
        Compute a small feature using global average pooling on X1 and/or X2.

        Args:
            X: (N, 3, H, W) in [0,1], RGB
            use_x1: include VGG-16 branch (512-D)
            use_x2: include SCNN branch (128-D)

        Returns:
            score: (N, 1) DBCNN scalar quality score (regular path)
            feat_small: (N, D_small) compact feature:
                - use_x1=True, use_x2=True  -> 640-D
                - use_x1=False, use_x2=True -> 128-D
        """
        assert use_x1 or use_x2, "At least one of use_x1/use_x2 must be True."

        X1, X2 = self._extract_backbone_features(X)

        # 1) Compute original DBCNN score (bilinear pooling inside)
        N, C1, H, W = X1.shape
        _, C2, _, _ = X2.shape
...
class DbcnnEncoder(nn.Module):
    """
    Drop-in encoder that outputs a 640-D DBCNN feature per frame (X1+X2).
    ...
    def __init__(self, use_x1=True, use_x2=True):
        ...
        # Freeze parameters (we just use it as a fixed encoder)
